class CrewApplicationsController < ApplicationController
  ...
  def new
    ...
    4.times do
      @crew_application.crew_wishes.build
    end
  ...
  def create
    ...
    @crew_application.event_id = active_event.id
  ...
  def crew_application_params
    params.require(:crew_application).permit(:id, :name, :address, :birthday, :email, :phone, :experience, :user_id,
                                             :event_id, crew_wishes_attributes: %i[id group_id priority])
  end
end
